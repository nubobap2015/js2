...
const myMainDiv = document.querySelector('.products');
...
/**
 * Эта функция вставляет товар в конец контрола.
 * @param {HTMLDivElement}
 * @param {Object} product объект из массива phones, tablets или tv.
 */
function insertProductIntoControl(mainControl, myProduct) {
    let myProductHtml = '<div class="product">';
    myProductHtml += `<div>${myProduct.name}</div>`;
    myProductHtml += `<img src="${myProduct.imageUrl}" alt="">`;
    myProductHtml += `<div>${myProduct.price}</div>`;
    myProductHtml += `<a href="https://example.com/producs/${myProduct.id}">Подробнее</a>`;
    myProductHtml += '</div>';
    mainControl.insertAdjacentHTML('beforeend',myProductHtml);
}
